...
**Files:**
- Create: `src/demo/pavoPackage.test.ts`
...
- [ ] **Step 3: 运行红测**
  ...
  Run: `rtk proxy npm test -- --run src/demo/pavoPackage.test.ts`
...
- [ ] **Step 5: 提交验收基线**
  ...
  Run: `rtk git add src/demo/pavoPackage.test.ts 短剧制作资料/17_正式Pavo制作包_v3.0/README.md 短剧制作资料/17_正式Pavo制作包_v3.0/00_总控/07_制作文件规范.md && rtk git commit -m "test: define v3 pavo package contract"`
...
- [ ] **Step 5: 运行针对性测试并提交**
  ...
  Run: `rtk proxy npm test -- --run src/demo/pavoPackage.test.ts src/demo/storyBaseline.test.tsx`
...
- [ ] **Step 3: 完成 B2-1—B2-3 与 B3-1—B3-3**

  B2 只做撤离与资产选择；B3 三条均保持重型设备永久留弃。

- [ ] **Step 4: 交叉核对、测试与提交**

  Run: `rtk proxy npm test -- --run src/demo/pavoPackage.test.ts src/demo/redesignedStory.test.ts`
...
- [ ] **Step 1: 完成 D0、D1、D2、D3 六文件**

  D0 报价声画不得暗示竞争者已获得机会；D1 不出现竞争者胜利；D2 交易物只出现地图、照片、坐标与勘探记录；D3 保持补给和调整缓冲接近归零。

- [ ] **Step 2: 完成 D1-1—D1-3、D2-1、D3-1—D3-3**

  D1 三条只做安全撤收；D2-1 明确资本回收与永久退出；D3 三条均保持重装留弃，其中 D3-3 伤势与执行能力代价最高。

- [ ] **Step 3: 交叉核对、测试与提交**

  Run: `rtk proxy npm test -- --run src/demo/pavoPackage.test.ts src/demo/redesignedStory.test.ts src/demo/storyBaseline.test.tsx`
