...
describe('keyframe review displays', () => {
  ...
  it('prefers the independent C finale K03 over its inherited parent K01', () => {
    const html = renderToStaticMarkup(<InheritedInputFrame nodeId="C3-2" parentNodeId="C3" scene="明确合作边界后按时提交" />)
    expect(html).toContain('data-finale-frame="C3-2"')
    expect(html).toContain('独立K03结果帧待验收')
    expect(html).toContain('generated-c3-2-k03-v1.png')
    expect(html).not.toContain('继承 C3 K03')
  })
  it('prefers the independent D finale K03 over its inherited parent K01', () => {
    const html = renderToStaticMarkup(<InheritedInputFrame nodeId="D3-3" parentNodeId="D3" scene="全队携更多样本截止前提交" />)
    expect(html).toContain('data-finale-frame="D3-3"')
    expect(html).toContain('独立K03结果帧待验收')
    expect(html).toContain('generated-d3-3-k03-v1.png')
    expect(html).not.toContain('继承 D3 K03')
  })
