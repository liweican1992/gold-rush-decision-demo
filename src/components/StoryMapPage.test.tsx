...
describe('story map page', () => {
  ...
  it('exposes the formal audit result and production gates for review', () => {
    ...
    expect(html).toContain('18 / 34 条结局已有独立K03结果候选')
    expect(html).toContain('其余 16 条继续显示继承K01输入帧')
  ...
  it('shows the ending story, final state and approximate profile on every finale card', () => {
    ...
    expect((html.match(/>独立K03结果帧待验收<\/span>/g) ?? []).length).toBe(18)
    expect((html.match(/K01输入帧待验收<\/span>/g) ?? []).length).toBe(16)
