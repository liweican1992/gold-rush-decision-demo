...
describe('keyframe eligibility', () => {
  ...
  it('registers every batch file conservatively with the exact six stages', () => {
    ...
    expect(data.assets).toHaveLength(81)
    expect(data.usages.every(row => row.acceptance === '未验收')).toBe(true)
    expect(data.usages.filter(row => row.placement === 'main').map(row => row.nodeId).sort()).toEqual(['A0', 'A3', 'B0', 'B1', 'B3', 'C0', 'C2', 'D0', 'D1', 'D3', 'INTRO', 'PRIMARY'])
    ...
    expect(getMain('A2', data)).toBeUndefined()
  })
